...
/**
 * MenuProductsController implements the CRUD actions for MenuProducts model.
 */
class MenuProductsController extends Controller
{
    ...
    {
        if (!$category = MenuCategories::findOne(['slug' => $slug])) {
            throw new NotFoundHttpException('Запрошенная вами страница не существует.');
        }

        $dataProvider = new ActiveDataProvider([
            'query' => MenuProducts::find()->andWhere(['category_id' => $category->id]),
    ...
    public function actionUpdate($id)
    {
        ...
        if ($model->load(Yii::$app->request->post())) {
            $model->category_id = intval(Yii::$app->request->post()["MenuProducts"]["category_id"]);
            $model->title_icon = $title_icon;
            $model->desc_icon = $desc_icon;
            $model->link_icon = $link_icon;
            if ($model->save()) {
    ...
    {
        if (($model = MenuProducts::findOne($id)) !== null) {
            return $model;
        }
    ...
    public function actionList($q = null, $id = null)
    {
        ...
        if (!is_null($q)) {
            ...
            $out['results'] = ['id' => $id, 'text' => MenuProducts::find($id)->title];
        }
        return $out;
    }
}
